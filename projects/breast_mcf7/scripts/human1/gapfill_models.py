...
for i, row in model_df.T.items():
    ...
    inactive_rxs = [k for k,v in activity.items() if not v]
    ...
    if sol.status() != 'optimal' or sol.objective_value() < 1e-3:
        ...
        gapfill_sol = gw.run(avbl_fluxes=inactive_rxs, algorithm='efm', ls_override={'produced':['temp001s']})
...
pickle_object(gapfill_results, os.path.join(CS_MODEL_DF_FOLDER, 'cs_models_all_combinations_efm_gapfill_open_media.csv'))
...
#
#     efm_gapfill_result[i] = gapfill_res[0] if len(gapfill_res) > 0 else []
#     print(i, gapfill_res)
